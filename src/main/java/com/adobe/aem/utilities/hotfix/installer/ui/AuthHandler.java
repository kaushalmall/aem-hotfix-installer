...
public class AuthHandler {
    ...
    private void loginTest() {
        ...
        try {
            ...
            Platform.runLater(()->{
                model.loginConfirmedProperty().set(false);
                model.loginErrorProperty().set(false);
                model.statusMessageProperty().set("Attempting connection");            
            });
            ...
            if (responseStatus.getStatusCode() >= 200 && responseStatus.getStatusCode() < 300) {
                Platform.runLater(()->{
                    ...
                    model.loginErrorProperty().set(false);
                    model.statusMessageProperty().set("Connected");
                });
            } else {
                Platform.runLater(()->{
                    model.loginConfirmedProperty().set(false);
                    model.loginErrorProperty().set(true);
                    model.statusMessageProperty().set("Error: " + responseStatus.getReasonPhrase() + " (" + responseStatus.getStatusCode() + ")");
                });
            }
            ...
            Platform.runLater(()->{
                model.statusMessageProperty().set("Error: " + ex.getMessage());
                model.loginErrorProperty().set(true);
                model.loginConfirmedProperty().set(false);
            });
        } catch (Throwable ex) {
